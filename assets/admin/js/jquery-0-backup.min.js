...
function open_model(order_id) {
    ...
    $.ajax({
        ...
        if (data.order_status == 2) {
			...
			var rider_list = '<table class="table table-stats"><tr><th></th><th>Rider Name</th><th>Orders</th><th>Assigned</th><th>Picking Add:</th><th>Drop Add:</th><th>Delivered</th></tr>';
            ...
            $.ajax({
                ...
                for (var i in data) {
					...
					//console.log(counter);
					//return false;
					rider_list=rider_list+'<tr><td><input type="radio" name="rider_id" value="'+counter.rider_id+'" /></td><td>'+counter.rider_name+' <br />('+counter.office_no+')</td>';
					if(counter.total_order){
					rider_list=rider_list+'<td>'+counter.total_order+'</td>';
					}else{
						rider_list=rider_list+'<td>'+counter.status+'</td>';
						}
					if(counter.last_assigned_time){
					rider_list=rider_list+'<td><abbr id="'+counter.lastassignedtime+'" class="timeago tip" title="'+counter.lastassignedtime+'" data-original-title="'+counter.lastassignedtime+'">'+counter.last_assigned_time+'</abbr></td>';
				}else{rider_list=rider_list+'<td>-</td>'; }
				
				rider_list=rider_list+'<td>'+counter.order_picking_address+'</td>';
				rider_list=rider_list+'<td>'+counter.order_drop_address+'</td>';
				rider_list=rider_list+'<td>'+counter.total_delivery+'</td>';
				
					rider_list=rider_list+'</tr>';
                    $('#rider_id').append($('<option />', {
                        value: counter.rider_id,
                        text: counter.rider_name + ' ('+ counter.office_no + ')'
                    }));
                }
				rider_list=rider_list+'</table>';
			//alert(rider_list);
    ...
    $.ajax({
        ...
        if (data.found) {
            ...
            for (var i in customer_locations) {
                ...
                picking_location_suggestion = picking_location_suggestion + '<span class="ms-sel-item"> ' + customer_location.name + ' <i class="fa fa-check" onclick="add_to_picking_location(\'' + customer_location.name + '\')"></i> </span>';
                delivery_location_suggestion = delivery_location_suggestion + '<span class="ms-sel-item"> ' + customer_location.name + ' <i class="fa fa-check" onclick="add_to_delivery_location(\'' + customer_location.name + '\')"></i> </span>';
