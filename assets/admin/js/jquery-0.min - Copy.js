...
function open_model(order_id) {
    ...
    $.ajax({
        ...
        var print_receipt = '<table style="font-size: 12px !important;" id="PrintOrder"><tr><td colspan="2"><h3 style="text-align: center;">' + title_print + '</h4></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Order ID:</strong></td><td> ' + order_id + ' (' + data.order_type + ')<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Customer:</strong></td><td> ' + data.customer_name + ' (' + data.mobile_number + ')<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td colspan="2" ><strong>Order Detail:</strong></td></tr><tr style="border: 1px solid gray !important;" ><td colspan="2"> ' + data.order_detail + '<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Picking Address:</strong></td><td> <i>' + data.order_picking_address + '</i><br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Delivery Address:</strong></td><td> <i>' + data.order_drop_address + '</i><br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Reg.</strong></td><td> <i>On ' + data.mobile_or_call;
        print_receipt += ' by ' + data.user_title + '';
        print_receipt += ' at ' + data.order_date_time + ' </i><br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Order On Name:</strong></td><td> ' + data.orderer_name + '<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Order Placed Time: </strong></td><td>' + data.order_place_time + '<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Order Ready Time: </strong></td><td> ' + data.order_ready_time + '<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Assigned Rider: </strong></td><td> ' + data.rider_name + '<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Assignded Time: </strong></td><td>' + data.order_rider_assign_time + '<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Acknowledged Time: </strong></td><td> ' + data.order_rider_acknowledge + '<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Order Picked Time </strong></td><td> ' + data.order_picking_time + '<br /></td></tr>';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Expected Delivery Time: </strong></td><td> ' + data.delivery_time + '<br />';
        print_receipt += '<tr style="border: 1px solid gray !important;" ><td><strong>Delivery Ready Time: </strong></td><td> ' + data.order_rider_delivery_time + '<br /></td></tr>';
        print_receipt += '<tr><td colspan="2"><i>Print By ' + data.print_by + '  on ' + data.print_time + '<i></td></tr></table>';
        ...
        if (data.order_type == 'General Order') {
            ...
            $('#order_type').html('<span class="label label-warning label-sm">' + data.order_type + '<span>');
        }
        if (data.order_type == 'Food Order') {
			$( "#food_order_radio_button" ).prop( "checked", true );
            $('#order_type').html('<span class="label label-success  label-sm">' + data.order_type + '<span>');
        }
        ...
        $('#created_by').html(data.user_title);
        ...
        if (data.order_status == 2) {
            ...
            $.ajax({
                ...
                for (var i in data) {
					...
					last_assigned_time='<abbr id="'+counter.lastassignedtime+'" class="timeago tip" title="'+counter.lastassignedtime+'" data-original-title="'+counter.lastassignedtime+'">'+counter.last_assigned_time+'</abbr>';
				}else{last_assigned_time=''; }
					
					 rider_table_rider_row.row.add( [
									'<input type="radio" name="rider_id" value="'+counter.rider_id+'" />',
...
function get_food_menu() {
    ...
    if (restaurant_id) {
        $.ajax({
            ...
            for (var i in data) {
                var counter = data[i];
                food_menu = counter.restaurant_food_category + ' | ' + counter.restaurant_food_name + ' | ' + counter.restaurant_food_quantity + ' | Rs: ' + counter.restaurant_food_price;
                $('#restaurant_food_menu_id').append($('<option>', {
...
function add_food_to_cart() {
    ...
    if (restaurant_food_menu_id) {
        $.ajax({
            type: "POST",
            url: site_url + "/restaurant_food_menus/add_food_to_cart/" + restaurant_food_menu_id,
            data: {}
        }).done(function(data) {
            //alert(data);
            data = jQuery.parseJSON(data);
...
function icrement_cart_item_quantity(item_cart_id) {
    ...
    $.ajax({
        type: "POST",
        url: site_url + "/restaurant_food_menus/icrement_cart_item_quantity/" + item_cart_id,
        data: {
            quantity: quantity
        }
    }).done(function(data) {
        ...
        if (data.found) {
           ...
           /* var picking_location = $("#picking_location").magicSuggest({});
            ...
            for (var i in customer_locations) {
                ...
                picking_location_suggestion = picking_location_suggestion + '<span class="ms-sel-item"> ' + customer_location.name + ' <i class="fa fa-check" onclick="add_to_picking_location(\'' + customer_location.id + '\')"></i> </span>';
                delivery_location_suggestion = delivery_location_suggestion + '<span class="ms-sel-item"> ' + customer_location.name + ' <i class="fa fa-check" onclick="add_to_delivery_location(\'' + customer_location.id + '\')"></i> </span>';
...
function get_all_mobile_numbers() {
    $.ajax({
        ...
        data = jQuery.parseJSON(data);
        //alert(data);
        var mobile_numbers = '<li style="color:red; cursor:pointer" onclick="get_all_mobile_numbers()"> <strong>Live Mobile No.</strong> </li>';
        for (var i in data) {
            var counter = data[i];
