...
<div class="jumbotron" style="padding: 2px;">
    ...
    INNER JOIN sub_components as sc ON(sc.component_id = `component_categories`.sub_component_id)
    ...
    foreach ($categories as $category) {
        ...
        $awp_target = $this->db->query($query, array($category['sub_component_id'], $fy_id))->row_array();
        ...
        $query = "SELECT COUNT(scheme_id) as total,
            ...
            WHERE sc.sub_component_id = ? AND financial_year_id IN ?";
        $schemes = $this->db->query($query, array($category['sub_component_id'], $fy_id))->row_array();
    ...
    <script>
        ...
        chart = Highcharts.chart('subComponentsTargets', {
            chart: {
                type: 'column'
            },
            title: {
