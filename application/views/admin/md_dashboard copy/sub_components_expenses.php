...
<div class="jumbotron" style="padding: 2px;">
    ...
    $query = "SELECT sc.sub_component_name as category, sc.sub_component_id FROM `component_categories` 
    INNER JOIN sub_components as sc ON(sc.sub_component_id = `component_categories`.sub_component_id)
    GROUP BY  sc.sub_component_id";
    $categories = $this->db->query($query)->result_array();

    foreach ($categories as $category) {
        $query = "SELECT SUM(total_cost) as total_cost FROM `annual_work_plans`
                  WHERE sub_component_id = ? AND financial_year_id IN ?";
        $awp_budget = $this->db->query($query, array($category['sub_component_id'], $fy_id))->row_array();
        ...
        $query = "SELECT SUM(net_pay) as total
            ...
            WHERE sc.sub_component_id = ? AND expenses.financial_year_id IN ?";
        $expenses = $this->db->query($query, array($category['sub_component_id'], $fy_id))->row_array();
    ...
    <div id="subComponentsExpenses" style="height: 300px;"></div>
    ...
    <script>
        ...
        chart = Highcharts.chart('subComponentsExpenses', {
            chart: {
                type: 'column'
            },
            title: {
