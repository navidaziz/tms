...
foreach ($financial_years as $financial_year) {
    ...
    $f_year_labels[] = "'" . $financial_year['financial_year'] . '<span class="f16"> <span id="flag" class="flag no"></span></span> ' . "'";

    $query = "SELECT SUM(net_pay) as total
    FROM expenses
    WHERE  expenses.financial_year_id = ?";
    $expenses = $this->db->query($query, array($financial_year['financial_year_id']))->row_array();
    ...
    $fy_budget = $this->db->query($query, array($financial_year['financial_year_id']))->row_array();
    if ($fy_budget) {
    ...
    $query = "SELECT SUM(rs_total) as br_total FROM `budget_released`
                  WHERE financial_year_id = ?";
    $budgetReleased = $this->db->query($query, array($financial_year['financial_year_id']))->row_array();
    if ($budgetReleased) {
    ...
    $query = "SELECT SUM(rs_total) as wb_total FROM `donor_funds_released`
                  WHERE financial_year_id = ?";
    $worldbankReleased = $this->db->query($query, array($financial_year['financial_year_id']))->row_array();
...
<div class="jumbotron" style="padding: 2px;">
    <div id="budgetUtilization" style="height:400px"></div>

    <script>
        Highcharts.chart('budgetUtilization', {
            ...
            xAxis: {
                ...
                lineWidth: 0,
                gridLineWidth: 0,
                categories: [<?php echo implode(', ', $f_year_labels); ?>]
            },
            yAxis: {
                lineWidth: 1,
                tickInterval: 1000,
